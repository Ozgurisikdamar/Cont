...
def evaluate_split(model, space: LabelSpace, texts: list[str], yg: np.ndarray, Ys: np.ndarray | None) -> dict:
    ...
    if Ys is not None:
        ...
        out["subtopics"] = multilabel_report(Ys, dec, joint_subtopic_probs(gp, cond, space.parent_col),
                                             space.subtopic_ids)
...
def errors(texts: list[str], yg: np.ndarray, gp: np.ndarray, labels: list[str], k: int) -> list[dict]:
    ...
    return [{"text": texts[i], "true": labels[yg[i]], "pred": labels[int(gp[i].argmax())],
             "confidence": round(float(gp[i].max()), 3)} for i in wrong[:k]]
...
def acceptance(model, tax) -> dict:
    ...
    for c in cases["single"]:
        ...
        single.append({**c, "pred_general": p.general, "confidence": round(p.confidence, 3),
                       "pred_subtopics": [(s.id, round(s.probability, 3)) for s in p.subtopics],
                       "status": p.status, "pass": bool(ok)})
    ood = []
    for c in cases["ood"]:
        p = model.predict(c["text"])
        ood.append({**c, "pred_general": p.general, "confidence": round(p.confidence, 3), "status": p.status,
                    "reasons": list(p.reasons), "ood_score": round(p.ood_score, 4), "pass": p.status == "uncertain"})
    settings = load_settings()
    tracker = ConversationTracker(decay=settings.decay, min_share=settings.theme_min_share,
    ...
    for i, msg in enumerate(cases["conversation"]["messages"], start=1):
        ...
        turn = {"turn": i, "text": msg, "pred": p.general, "theme_label": theme.label, "phrase": theme.phrase,
...
def measure_latency(model, texts: list[str]) -> dict:
    ...
    tracemalloc.stop()
    return {"single_ms_median": round(float(np.median(times)), 2), "single_ms_p95": round(float(np.percentile(times, 95)), 2),
            "batch_ms_per_text": round(batch_ms, 3), "python_heap_peak_mb_during_batch": round(peak, 1)}
...
def main() -> int:
    ...
    try:
        ...
        log.error("%s", exc)
        return 2
    report: dict = {"model": {k: model.metadata.get(k) for k in ("model_name", "model_version", "trained_at",
                                                                  "dataset_version", "encoder", "encoder_revision")},
                    "load_seconds": round(load_seconds, 2)}

    test = df[df.split == "test"]
    t_texts = [normalize(t) for t in test.text]
    res_test = evaluate_split(model, space, t_texts, space.encode_general(test.general),
                              space.encode_subtopics(test.subtopics))
    report["wiki_test"] = strip_private(res_test)
    report["wiki_test_errors"] = errors(t_texts, space.encode_general(test.general), res_test["_gp"],
                                        space.general_ids, N_ERROR_EXAMPLES)
    ...
    report["se_general_errors"] = errors(s_texts, space.encode_general(se_test.general), res_se["_gp"],
                                         space.general_ids, N_ERROR_EXAMPLES)
    sub_test = ses[ses.split == "ext_test"]
    res_sesub = evaluate_split(model, space, [normalize(t) for t in sub_test.text],
                               space.encode_general(sub_test.general), space.encode_subtopics(sub_test.subtopics))
    ...
    report["calibration"] = {"wiki_test": reliability_curve(res_test["_gp"], yg_test),
    ...
    plot_confusion(report["se_general_ext_test"]["general"], figs / "confusion_se_ext_test.png",
                   "General topic - Stack Exchange ext_test")
    plot_per_class_f1(report["wiki_test"]["subtopics"]["per_label"], figs / "subtopic_f1_wiki_test.png",
    ...
    log.info("wiki test acc=%.4f macroF1=%.4f | SE ext_test acc=%.4f macroF1=%.4f | wrote %s",
             g["accuracy"], g["macro_f1"], s["accuracy"], s["macro_f1"], out)
    return 0
