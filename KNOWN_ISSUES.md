...
| 12 | **Latency depends on machine load** | low | CPU inference | median 19.9 ms idle; one test run measured 0.52 s while fine-tuning used all cores | – | none for a single-user app |
| 13 | **E-6 (larger fine-tuned encoder) not run** | low | owner decided to proceed with MiniLM | unknown whether a fine-tuned e5/bge would be better on questions | – | fine-tune e5-small-v2 with the same script and compare on ext_dev |
